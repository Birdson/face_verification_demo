...
CaffeFaceDetection::~CaffeFaceDetection() {
}

std::vector<vector<float>> CaffeFaceDetection::Detect(const cv::Mat& img) {
  Blob<float>* input_layer = net_->input_blobs()[0];
...
void CaffeFaceDetection::SetMean(const string& mean_file, const string& mean_value) {
 ...
 * layer. */
void CaffeFaceDetection::WrapInputLayer(std::vector<cv::Mat>* input_channels) {
  Blob<float>* input_layer = net_->input_blobs()[0];
...
void CaffeFaceDetection::Preprocess(const cv::Mat& img,
                            std::vector<cv::Mat>* input_channels) {
